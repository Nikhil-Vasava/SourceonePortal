...
export async function approvePoAction(formData) {
  ...
  await record({
    action: "APPROVE", entity: "PurchaseOrder", entityId: id, label: po.number,
    summary: `Approved ${po.number} — the PDF now carries the company stamp and can be emailed`,
  });
...
export async function unapprovePoAction(formData) {
  ...
  await record({
    action: "UNAPPROVE", entity: "PurchaseOrder", entityId: id, label: po.number,
    summary: `Withdrew approval from ${po.number} — the stamp is off the PDF and staff can no longer email it`,
  });
