...
/**
 * The logo mark for the PO header.
 *
 * PNG rather than SVG because pdf-lib embeds raster images only — this is the
 * "where vectors aren't supported" case the brand guide allows. 256px is well
 * above the ~48pt it prints at, so it stays crisp.
 *
 ...
 */
function readLogo() {
  try { return fs.readFileSync(path.join(process.cwd(), "public", "logo-mark-256.png")); }
  catch { return null; }
}
...
async function buildPoPdf(po, company) {
  ...
  // ---------- Company header ----------
  // The mark sits at the left margin; the name block stays centred on the page
  // so the layout suppliers already recognise doesn't shift.
  ...
  y -= 30;
  text(spacedName, M + (BOX_W - nameW) / 2, y, { font: bold, size: nameSize, color: COLORS.blue });

  if (logo) {
    const size = 52;
    // Vertically centred against the name + legal name pair rather than the
    // name alone, so it reads as anchoring the whole block.
    page.drawImage(logo, {
      x: M + 4,
      y: y - size + nameSize - 6,
      width: size,
  ...
  y -= 20;
  text(spacedLegal, M + (BOX_W - legalW) / 2, y, { font: bold, size: legalSize, color: COLORS.blue });

  const addrSize = 8.5;
  const addrW = reg.widthOfTextAtSize(S(company.address || ""), addrSize);
  y -= 15;
  text(company.address, M + (BOX_W - addrW) / 2, y, { size: addrSize, color: COLORS.blue });
