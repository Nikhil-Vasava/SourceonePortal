...
async function buildPoPdf(po, company) {
  ...
  // The stamp stands on its own — no "Approved by …" caption underneath.
  // Naming the approver on the document told the supplier something internal
  // without adding anything they can act on; the stamp already says the order
  // is authorised. Who approved it, and when, is still recorded against the
  // order and shown on the Purchase tab and in the audit trail.
  const STAMP_W = 96;
  ...
  // then clamping the image into it is what pushed the stamp out of its space.
  y -= Math.max(92, stampH + 24);
