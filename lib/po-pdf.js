 ...
 */
function readStamp() {
  try { return fs.readFileSync(path.join(process.cwd(), "public", "stamp-approved.png")); }
  catch { return null; }
}
...
const COLORS = {
  ...
  black: rgb(0, 0, 0),
  line: rgb(0, 0, 0),
  grey: rgb(0.42, 0.42, 0.42),   // the approval caption under the stamp
};
...
async function buildPoPdf(po, company) {
  ...
  //
  // Off by default while a new logo is being designed. The name block is
  // centred on the page independently of the mark, so omitting it leaves the
  // letterhead balanced rather than lopsided — nothing else needs to move.
  const logoBytes = company?.showPoLogo ? readLogo() : null;
  if (logoBytes) {
    let logo = null;
  ...
  //
  // Both columns wrap and the row grows to fit the taller one. This used to
  // draw the left value as a single unbounded line, so "New Zealand Document
  // Destruction Services (Rotorua)" ran straight through the divider and
  // printed on top of the word "Address".
  //
  // The gutters are measured from the labels actually used rather than being
  // guessed at, so adding a longer label later widens the column instead of
  // colliding with the colon.
  const ROW_H = 18;
  const LINE_H = 11;
  const FS = 8.5;
  const PAD = 8;

  const LEFT_LABELS  = ["GST / HST No.", "Import / Export No.", "P.O. No.", "Vendor Name", "Phone No."];
  const RIGHT_LABELS = ["Phone No.", "E-mail ID", "Date", "Address"];
  const widest = (labels) =>
    Math.max(...labels.map(l => bold.widthOfTextAtSize(S(l), FS)));

  const LBL_X  = M + 6;
  const COL_X  = LBL_X + widest(LEFT_LABELS) + 6;   // the colon
  const VAL_X  = COL_X + 8;
  const LVAL_W = MID - VAL_X - PAD;

  const RLBL_X = MID + 6;
  const RCOL_X = RLBL_X + widest(RIGHT_LABELS) + 6;
  const RVAL_X = RCOL_X + 8;
  const RVAL_W = (M + BOX_W) - RVAL_X - PAD;

  function infoRow(l1, v1, l2, v2) {
    const left  = wrap(S(v1 ?? ""), reg, FS, LVAL_W);
    const right = l2 === null ? [] : wrap(S(v2 ?? ""), reg, FS, RVAL_W);

    const rows = Math.max(left.length, right.length, 1);
    const height = rows === 1 ? ROW_H : rows * LINE_H + 10;

    const top = y;
    y -= height;

    // One line sits centred in the row; several start from the top so the two
    // columns read side by side instead of drifting apart.
    const firstLine = rows === 1 ? y + height / 2 - 3 : top - 12;

    text(l1, LBL_X, firstLine, { font: bold, size: FS });
    text(":", COL_X, firstLine, { font: bold, size: FS });
    left.forEach((ln, i) => text(ln, VAL_X, firstLine - i * LINE_H, { size: FS }));

    if (l2 !== null) {
      text(l2, RLBL_X, firstLine, { font: bold, size: FS });
      text(":", RCOL_X, firstLine, { font: bold, size: FS });
      right.forEach((ln, i) => text(ln, RVAL_X, firstLine - i * LINE_H, { size: FS }));
    }
  ...
  infoRow("Vendor Name", po.vendorName, "Address", po.vendorAddress);
  ...
  // ---------- Terms & conditions ----------
  // Free text now, so it wraps and the box grows with it. The stamp sits on the
  // right, so the text is kept clear of that side to avoid running underneath.
  y -= 18;
  text("TERMS & CONDITIONS :", LBL_X, y + 6, { font: bold, size: 8.5 });
  y -= 4;

  const TERMS_W = BOX_W - (LBL_X - M) - 175;   // 175pt reserved for the stamp
  const termsText = po.terms
    || (po.minimumWeight ? `- Minimum Weight ${po.minimumWeight}` : "");

  // Author's line breaks are meaningful — each condition is its own bullet — so
  // wrap within each line rather than reflowing the whole block as one blob.
  for (const rawLine of String(termsText).split(/\r?\n/)) {
    const line = rawLine.trimEnd();
    if (!line) { y -= 6; continue; }

    const words = line.split(/\s+/);
    let buf = "";
    // Continuation lines indent under the bullet's text, not under the dash.
    const indent = /^\s*-/.test(rawLine) ? 8 : 0;
    let first = true;

    const flush = () => {
      y -= 12;
      text(buf, LBL_X + (first ? 0 : indent), y + 3, { size: 8.5 });
      first = false;
      buf = "";
    };
    ...
    for (const w of words) {
      ...
      if (reg.widthOfTextAtSize(test, 8.5) <= TERMS_W - (first ? 0 : indent)) { buf = test; continue; }
      if (buf) flush();
      buf = w;
    }
    if (buf) flush();
  ...
  // ---------- Approval stamp ----------
  // Drawn only for an approved order. An unapproved PO must come out of the
  // system unsigned — the stamp is what tells a supplier this was authorised,
  // so it can't appear on a draft somebody merely printed.
  const STAMP_W = 96;
  const CAPTION = 10;      // room for the "Approved by …" line under the stamp
  let stamp = null, stampH = 0;

  if (po.approved) {
    const stampBytes = readStamp();
    if (stampBytes) {
      try {
        stamp = await doc.embedPng(stampBytes);
        stampH = (stamp.height / stamp.width) * STAMP_W;
      } catch { stamp = null; }   // an unreadable stamp must not break the document
    }
  }

  // Size the space to whatever has to go in it. Fixing the height first and
  // then clamping the image into it is what pushed the stamp onto its caption.
  const stampTop = y;
  y -= Math.max(92, stampH + CAPTION + 20);

  if (stamp) {
    // Right-hand side, clear of the terms text — which is already kept 175pt
    // off that edge for exactly this reason.
    const sx = M + BOX_W - STAMP_W - 42;
    const sy = y + CAPTION + 8;
    page.drawImage(stamp, { x: sx, y: sy, width: STAMP_W, height: stampH, opacity: 0.92 });

    const who = po.approvedName ? `Approved by ${po.approvedName}` : "Approved";
    const label = who + (po.approvedOn ? `  ·  ${po.approvedOn}` : "");
    const lw = reg.widthOfTextAtSize(label, 7);
    // Centred under the stamp, but never allowed past the left edge of the box.
    text(label, Math.max(M + 6, sx + (STAMP_W - lw) / 2), y + 4, { size: 7, color: COLORS.grey });
  }
