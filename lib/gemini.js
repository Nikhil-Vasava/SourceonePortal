 ...
 *
 * Document-agnostic: the prompt and schema live in extract.js so every kind of
 * document (packing slips, booking confirmations) shares the same retry,
 * model-fallback and quota handling.
 */
export async function extractDocumentRaw({ prompt, schemaHint, base64, mimeType }) {
  return callGemini({ prompt, base64, mimeType, schemaHint });
}

/** @deprecated Kept so existing callers don't break; use extractDocumentRaw. */
export async function extractPackingSlipRaw(args) {
  return extractDocumentRaw(args);
}
