...
import { isWordDoc, docxAsTextPart } from "@/lib/docx-text";
import { isSpreadsheet, xlsxAsTextPart } from "@/lib/xlsx-text";
import { assertUploadType } from "@/lib/upload-types";
import { deriveErd } from "@/lib/booking-parsers";
...
export async function extractPackingSlip(base64, fileName) {
  ...
  // Reject anything we can't handle before spending a Gemini call on it. This
  // covers .doc, spreadsheets, archives and anything else that gets past the
  // file picker by being renamed or dragged in.
  assertUploadType(fileName, "packingSlip");

  // Gemini takes PDFs and images only, so a .docx or .xlsx becomes text first.
  // Conversion errors are thrown as-is — they already explain what to do and
  // aren't Gemini's doing.
  ...
  if (isWordDoc(fileName)) {
    ...
    part = await xlsxAsTextPart(Buffer.from(base64, "base64"));
  }
...
const BOOKING_SCHEMA = `{
  ...
  "emptyDepot": "where the EMPTY container is collected — the depot or container park name and its street address on one line, e.g. 'CC Woolston Container Park (WCP), 69 Chapmans Road, Lyttelton'. Labelled 'Empty Container Depot', 'Empty Pick UP CY', or 'PICK UP DEPOT ADDRESS' depending on the carrier. NOT the return depot, NOT the port terminal, NOT a PO Box. null if the document does not say",
  "containers": [
...
export async function extractBookingWithGemini(base64, fileName) {
  ...
  // readBookingDocument already screens the file type, but this is exported and
  // callable on its own, so it checks too rather than trusting its caller.
  assertUploadType(fileName, "booking");
...
function normaliseBooking(raw) {
  ...
  return {
    ...
    // ERD is calculated from ETD, not read — the model isn't asked for it, and
    // anything it volunteered under that name is ignored. See deriveErd().
    erd: deriveErd(s(raw?.etd)),
    docsCutOff: s(raw?.docsCutOff),
    cargoCutOff: s(raw?.cargoCutOff),
    etd: s(raw?.etd),
    eta: s(raw?.eta),
    emptyDepot: s(raw?.emptyDepot),
    containers,
  };
}
