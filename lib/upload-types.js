...
export const UPLOAD_KINDS = {
  ...
  packingSlip: {
    label: "packing slip",
    extensions: ["pdf", ...IMAGES, "docx", "xlsx"],
    advice: "Upload a PDF, a photo (JPG, PNG or HEIC), a Word .docx, or an Excel .xlsx.",
  },
...
const KNOWN_PROBLEMS = {
  ...
  xls: "Old-style Excel files (.xls) can't be read. Open it in Excel and use " +
       "File → Save As to save an .xlsx or a PDF, then upload that.",
...
export function assertUploadType(fileName, kind) {
  ...
  // Same idea for .xlsx sent somewhere that doesn't take spreadsheets.
  if (ext === "xlsx") {
    const takesExcel = Object.values(UPLOAD_KINDS)
      .filter(k => k !== spec && k.extensions.includes("xlsx"))
      .map(k => k.label);
    const where = takesExcel.length
      ? ` Excel files are only read on the ${takesExcel.join(" and ")} upload.`
      : "";
    throw new Error(`Excel files can't be read as a ${spec.label}. ${spec.advice}${where}`);
  }
