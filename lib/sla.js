...
/**
 * The empty-container pickup window.
 *
 * Opens 14 days before ERD and closes on it: the run-up in which operations
 * have to arrange collection of the empties and the supplier has to have the
 * material ready.
 *
 * It gets its own warn threshold rather than reusing `slaWarnDays`. That's set
 * to 40, which on a 14-day window can never be reached before the clock is
 * already breached — the band would jump from "on track" straight to
 * "overdue" and never once say "follow up".
 */
export const PICKUP = { days: 14, warnDays: 11 };
...
export function bookingClocks(booking, settings = SLA_DEFAULTS, now = new Date()) {
  ...
  // Empty-container pickup: counts down to ERD from 14 days out. Stops once a
  // packing slip has arrived for the booking, which is this app's evidence the
  // containers were actually filled.
  const pickupStart = pickupWindowOpens(booking.erd);
  const packed = (booking.lines || []).some(l => l.packingSlipFile);
  const packedAt = packed
    ? (booking.lines.map(l => l.packingDate).filter(Boolean).sort()[0] || booking.erd || new Date())
    : null;
  const pickup = clock(pickupStart, packedAt, PICKUP.days, PICKUP.warnDays, now);
  ...
  return {
    ...
    // Sorting and filtering use the more urgent of the two contractual clocks.
    // Pickup is deliberately excluded — it's an internal prompt, not a promise
    // to a carrier or a buyer, and mixing it in would drown the two that are.
    worst: severity(carrier) >= severity(buyer) ? carrier : buyer,
  };
}

/** The day the 14-day pickup window opens, or null without an ERD. */
export function pickupWindowOpens(erd) {
  if (!erd) return null;
  const d = startOfDay(erd);
  d.setDate(d.getDate() - PICKUP.days);
  return d;
}
