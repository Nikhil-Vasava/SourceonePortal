...
import { settingsUnlocked } from "@/lib/settings-lock";
import { record } from "@/lib/audit";

/**
 * Admin role plus a live PIN unlock. Hiding the buttons isn't a control — a
 * server action is a public endpoint to anyone who knows its id — so the PIN is
 * re-checked here rather than trusted from the page that rendered the button.
 */
function requireUnlockedAdmin() {
  requireRole("ADMIN");
  if (!settingsUnlocked()) {
    throw new Error("Settings is locked. Enter your PIN again and retry.");
  }
}
...
export async function clearPackingSlipsAction() {
  ...
  await record({
    action: "RESET", entity: "Setting", label: "Packing slips",
    summary: `Cleared packing slip details from ${count} container line${count === 1 ? "" : "s"}`,
  });
...
export async function clearBuyerAllocationsAction() {
  ...
  await record({
    action: "RESET", entity: "Setting", label: "Buyer allocations",
    summary: `Cleared buyer allocation from ${count} container line${count === 1 ? "" : "s"}`,
  });
...
export async function deleteAllBookingsAction() {
  ...
  // The single most destructive thing the app can do. The audit row is written
  // last so it survives the transaction rolling back — and it survives the
  // deletion itself, because nothing in the trail is ever removed.
  await record({
    action: "RESET", entity: "Setting", label: "All bookings & POs",
    summary: `Deleted every booking and purchase order — ${bookings} booking${bookings === 1 ? "" : "s"}`
           + ` and ${pos} purchase order${pos === 1 ? "" : "s"} removed`,
  });
