...
export async function createBookingFromExtract(data, fileName) {
  ...
  return prisma.booking.create({
    data: {
      ...
      // Where the empty box is collected. Feeds the 14-day pickup notice, which
      // is the one email that exists to send someone to this address.
      emptyDepot: data.emptyDepot || null,
      erd: toDate(data.erd),
...
export async function readBookingDocument(buffer, fileName) {
  // Checked before any parsing so an unreadable format fails with advice
  // rather than "no text found" or a raw MIME rejection from Gemini.
