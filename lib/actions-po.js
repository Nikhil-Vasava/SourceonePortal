...
import { bookingCapacity } from "@/lib/po-allocation";
...
export async function linkPoAction(formData) {
  ...
  const [po, booking] = await Promise.all([
    ...
    prisma.booking.findUnique({ where: { id: bookingId }, include: { lines: true } }),
  ]);
  if (!po || !booking) return;

  // How much this shipment takes off the order. Read from the booking rather
  // than asked for — the app already knows, and a 40-load PO going out as
  // 10 + 7 + 15 + 8 shouldn't need anyone doing the subtraction.
  const qty = bookingCapacity(booking);

  await prisma.poAllocation.upsert({
    where: { poId_bookingId: { poId, bookingId } },
    create: { poId, bookingId, qty, unit: po.lines[0]?.uom ?? null },
    // Re-linking refreshes the figure, so upsizing a booking from 10 to 12
    // containers corrects the balance instead of double-counting it.
