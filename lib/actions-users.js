...
export async function createUserAction(formData) {
  ...
  await record({
    action: "CREATE", entity: "User", entityId: created.id, label: name,
    summary: `Created account for ${name} (${addr}) with the ${role} role`,
  });
...
export async function updateUserAction(formData) {
  ...
  const after = { name, email: addr, role, region: clean(formData.get("region")) || null };
  await prisma.user.update({ where: { id }, data: after });

  await recordUpdate({
    entity: "User", entityId: id, label: name, noun: "account",
    before: target, after,
  });
...
export async function setUserPasswordAction(formData) {
  ...
  // Records that it happened, never what it was set to. The same reasoning as
  // the message below, and lib/audit.js refuses the password field outright.
  await record({
    action: "UPDATE", entity: "User", entityId: id, label: target.name,
...
export async function toggleUserActiveAction(formData) {
  ...
  await record({
    ...
    summary: target.active
      ? `Disabled ${target.name} — they can no longer sign in`
      : `Re-enabled ${target.name}`,
    changes: [{ field: "active", from: target.active ? "yes" : "no", to: target.active ? "no" : "yes" }],
  });
