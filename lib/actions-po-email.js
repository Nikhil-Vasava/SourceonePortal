...
import { requireUser, canHandlePurchaseOrders } from "@/lib/auth";
...
export async function poDraftAction(poId) {
  ...
  if (!canHandlePurchaseOrders(viewer)) return { error: "You don't have access to purchase orders." };
  ...
  return {
    ...
    configured: mailConfigured("PO"),
    missing: mailConfigured("PO") ? [] : missingMailSettings("PO"),
    ...
    aliasSend: fromDiffersFromLogin("PO"),
...
export async function sendPoEmailAction(formData) {
  ...
  // Only the purchase team sends purchase orders. The document is priced,
  // so dispatching one is a commercial act even though the sender never
  // opens it.
  if (!canHandlePurchaseOrders(user)) {
    return { error: "Only the purchase team can send purchase orders." };
  }

  const id = Number(formData.get("id"));
  if (!Number.isFinite(id)) return { error: "Missing purchase order." };

  // Nothing unapproved goes to a supplier — no exception for admins.
  //
  // An unapproved PO renders without the company stamp, and a supplier has no
  // way to tell an unstamped draft from the real thing. Approving is one click
  // for the people who'd otherwise be exempt, so the rule costs nothing and
  // removes the possibility entirely.
  //
  // Checked here rather than only in the table: a hidden button stops a click,
  // not a request, and this action is reachable without one.
  ...
  if (!gate.approvedAt) {
    return {
      error: user.role === "ADMIN"
        ? `${gate.number} isn't approved yet, so its PDF has no stamp. Approve it first, then send.`
        : `${gate.number} hasn't been approved yet. An admin needs to approve it before it can be sent.`,
    };
  }
  ...
  try {
    ...
    // Recorded after the send succeeds, not before: an entry saying an order
    // went to a supplier when it didn't is worse than no entry at all.
    await record({
      action: "EMAIL", entity: "PurchaseOrder", entityId: id, label: gate.number,
      summary: `Emailed ${gate.number} to ${everyone.join(", ")}`,
    });
