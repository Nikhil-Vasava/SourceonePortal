...
export async function savePartnerAction(formData) {
  ...
  if (before) {
    await recordUpdate({
      entity: "Partner", entityId: partner.id, label: name, noun: "contact",
      before, after: data,
    });
  } else {
    await record({
      action: "CREATE", entity: "Partner", entityId: partner.id, label: name,
      summary: `Added ${name} to Info as a ${String(data.type).toLowerCase().replace("_", " ")}`,
    });
  }
...
export async function deletePartnerAction(formData) {
  ...
  await record({
    action: "DELETE", entity: "Partner", entityId: id, label: partner.name,
    summary: `Deleted ${partner.name} from Info`,
  });
...
export async function togglePartnerActiveAction(formData) {
  ...
  await record({
    action: "UPDATE", entity: "Partner", entityId: id, label: partner.name,
    summary: partner.active
      ? `Deactivated ${partner.name} — hidden from dropdowns, existing records untouched`
      : `Reactivated ${partner.name}`,
    changes: [{ field: "active", from: partner.active ? "yes" : "no", to: partner.active ? "no" : "yes" }],
  });
...
export async function saveProductAction(formData) {
  ...
  if (beforeProduct) {
    await recordUpdate({
      entity: "Product", entityId: product.id, label: product.name, noun: "product",
      before: beforeProduct, after: data,
    });
  } else {
    await record({
      action: "CREATE", entity: "Product", entityId: product.id, label: product.name,
      summary: `Added product ${product.name} (${sku})`,
    });
  }
...
export async function deleteProductAction(formData) {
  ...
  await record({
    action: "DELETE", entity: "Product", entityId: id, label: product.name,
...
export async function toggleProductActiveAction(formData) {
  ...
  await record({
    action: "UPDATE", entity: "Product", entityId: id, label: product.name,
    summary: product.active
      ? `Deactivated product ${product.name}`
      : `Reactivated product ${product.name}`,
    changes: [{ field: "active", from: product.active ? "yes" : "no", to: product.active ? "no" : "yes" }],
  });
...
/**
 * The two contractual windows and the point at which to start chasing.
 *
 * Kept as company settings rather than constants because a renegotiated
 * contract shouldn't need a redeploy. The warning threshold has to sit below
 * the shorter of the two limits, otherwise it could never fire.
 */
export async function saveSlaAction(formData) {
  requireUser();

  const carrier = Number(formData.get("carrierSlaDays"));
  const buyer = Number(formData.get("buyerSlaDays"));
  const warn = Number(formData.get("slaWarnDays"));

  for (const [label, v] of [["Carrier days", carrier], ["Buyer days", buyer], ["Warning day", warn]]) {
    if (!Number.isInteger(v) || v < 1 || v > 365) {
      throw new Error(`${label} must be a whole number between 1 and 365.`);
    }
  }
  if (warn >= Math.min(carrier, buyer)) {
    throw new Error(
      `The warning day (${warn}) must be lower than the shorter window ` +
      `(${Math.min(carrier, buyer)} days), or it would never give you notice.`
    );
  }

  const company = await prisma.companySetting.findFirst();
  if (!company) throw new Error("Company settings are missing.");

  await prisma.companySetting.update({
    where: { id: company.id },
    data: { carrierSlaDays: carrier, buyerSlaDays: buyer, slaWarnDays: warn },
  });

  revalidatePath("/settings");
  revalidatePath("/tracking");
  revalidatePath("/");
  return { ok: true, message: `Now chasing from day ${warn}; carrier ${carrier} days, buyer ${buyer} days.` };
}
