...
export function readTableQuery(searchParams = {}, { defaultSort, defaultDir = "desc", perPage = 0 } = {}) {
  ...
  return {
    ...
    page: Number.isFinite(n) && n >= 1 ? Math.floor(n) : 1,
    perPage,
  };
}

/**
 * Slices already-sorted rows into one page.
 *
 * Paging happens after sorting rather than in the query because several
 * sortable columns come off relations and are sorted in memory — a database
 * LIMIT would take the wrong rows. These pages hold one office's shipments, so
 * the cost of loading them is far smaller than the cost of getting it wrong.
 *
 * A page number past the end clamps to the last page instead of showing an
 * empty table, which is what happens when the row count shrinks under a
 * bookmarked link.
 */
export function paginate(rows, query) {
  const per = query.perPage || 0;
  if (!per || rows.length <= per) {
    return { rows, page: 1, pages: 1, total: rows.length, from: rows.length ? 1 : 0, to: rows.length };
  }
...
/** Link to another page, carrying the current filters and sort. */
/**
 * @param extra any filters this screen has of its own — the audit trail filters
 *   by person, area and action. Without carrying them, clicking page 2 would
 *   silently drop the filter and show a different list than the one being paged.
 */
export function pageHref(basePath, query, page, extra = null) {
  const params = new URLSearchParams();
  if (query.q) params.set("q", query.q);
  if (query.from) params.set("from", query.from);
  if (query.to) params.set("to", query.to);
  if (query.sort) { params.set("sort", query.sort); params.set("dir", query.dir); }
